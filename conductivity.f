...
C Write header line for first line of output
...
C Cold slab = slab number 1
C Hot slab = slab in the middle
      ...
      do i=1,natom
        ...
        atom_ke=0.5*fracmass(i)*(vx(i,1)**2+vx(i,2)**2+vx(i,3)**2)
      ...
      open(unit=8,file='thermal.out',status='unknown',position='append')

C Check that the swap is correct
        ehot=fracmass(ihot)*(vx(ihot,1)**2+vx(ihot,2)**2+vx(ihot,3)**2)
        ecold=(vx(icold,1)**2+vx(icold,2)**2+vx(icold,3)**2)
        ecold=fracmass(icold)*ecold

        if(ehot.le.ecold) then
          write(6,*) 'warning, a velocityswap in the wrong direction'
          write(8,*) 'warning, a velocityswap in the wrong direction'
        endif

C Swapping the velocities of the hot and cold atoms
      do ind=1,3
        vnewhot=(fracmass(ihot)-fracmass(icold))*vx(ihot,ind)
        vnewhot=vnewhot+2*fracmass(icold)*vx(icold,ind)
        vnewhot=vnewhot/(fracmass(icold)+fracmass(ihot))

        vnewcold=(fracmass(icold)-fracmass(ihot))*vx(icold,ind)
        vnewcold=vnewcold+2*fracmass(ihot)*vx(ihot,ind)
        vnewcold=vnewcold/(fracmass(icold)+fracmass(ihot))

        vx(ihot,ind)=vnewhot
        vx(icold,ind)=vnewcold
      end do

      vflag=1.0
      swap_ke=swap_ke + hot_ke - cold_ke

C Adjust KE of cold & hot slabs, and convert to Kelvin
...
C Write temperature of slabs to file
