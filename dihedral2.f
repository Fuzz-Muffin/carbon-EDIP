...
c +-----------------------------------+
c | setup vectors: 1=ij , 2=ik , 3=jm |
c +-----------------------------------+
	    do ind=1,3
...
c +-------------------------------------------------------------+
c | g3(Zi)*g3(Zj) * (ahat . bhat x chat)^2 * f(a) * f(b) * f(c) |
c +-------------------------------------------------------------+
	    ...
	    z(i)=z(i) + 0.5d0*zdih * gg * dot*dot * ff3
	    do ind=1,3
...
c +-----------------------------------------------------+
c | if neighbour of i: dg3(Zi) * g3(Zj) * dot*dot * ff3 |
c +-----------------------------------------------------+
	    do nni=1,num(i)
	      if ((nni.ne.jj).and.(nni.ne.kk)) then
	      if (dr(i,nni).lt.zhigh) then
	        finiteforce(nni)=.true.
                fac=0.5d0*zdih * dg3(i)*dzz(i,nni) *g3(j)* dot*dot * ff3
	        do ind=1,3
...
c +-----------------------------------------------------+
c | if neighbour of j: dg3(Zj) * g3(Zi) * dot*dot * ff3 |
c +-----------------------------------------------------+
	    do nnj=1,num(j)
	      if ((near(j,nnj).ne.i).and.(nnj.ne.mm)) then
	      if ((dr(j,nnj).lt.zhigh)) then
	        finiteforce2(jj,nnj)=.true.
	        fac=0.5d0*zdih*dg3(j) * dzz(j,nnj) *g3(i)* dot*dot * ff3
		do ind=1,3
