...
c +-----------------------------------+
c | Dummy routine when special=.true. |
c +-----------------------------------+
 50   natom=1
      t0=0
      do ind=1,3
...
c +----------------------------------------------+
c | (ntakof=0) Starting from atom positions only |
c +----------------------------------------------+
 100  open(unit=7,file='START',status='old')
      read(7,*) natom
      if (natom.gt.NMAX) stop 'Increase NMAX'
...
c +---------------------------------------------------------------+
c | (ntakof=1) Continue on from existing phase point, restart MSD |
c +---------------------------------------------------------------+
 200  open(unit=7,file='START',status='old')
      read(7,210) t0,istep,natom
      if (natom.gt.NMAX) stop 'Increase NMAX'
...
c +---------------------------------------------------------------+
c | (ntakof=2) Continue on from existing phase point and MSD data |
c +---------------------------------------------------------------+
 300  open(unit=7,file='START',status='old')
      read(7,310) t0,istep,natom
      if (natom.gt.NMAX) stop 'Increase NMAX'
